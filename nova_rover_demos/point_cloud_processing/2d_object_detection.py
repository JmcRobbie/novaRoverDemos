...
class PointCloudGen:
    ...
    def getObjects(self):
        '''
        Segments 2D occupancy grid into distinct objects. In the form of lists of points belonging to each object.
        This can then be turned into desired representations such as rects/boxes or cylinders to represent the object
        '''
...
def drawCylinder(ax, center_x,center_y,radius,height_z):
...
def test_object_detection():
    size = [(0, 1), (0, 2), (0, 5)]
    cld = PointCloudGen(size, 3)
    cld.gaussian_cloud()
    cld.get_occupancy_grid(0.05, 0.05, True)

    # #############################################################################
    # Compute DBSCAN
    db = DBSCAN(eps=0.1, min_samples=10).fit(cld.series)
    ...
    for i in range(n_clusters_):
        ...
        points = np.array(cld.series)[np.where(labels == i)]
        ...
        drawCylinder(ax, center[0], center[1],radius,0.1)


    X = np.array(cld.series)[:, 0]
    Y = np.array(cld.series)[:, 1]
